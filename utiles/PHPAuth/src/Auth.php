...
final class Auth extends UserManager {
	...
	public function resendConfirmationForEmail($email, callable $callback) {
		...
		//return $this->resendConfirmationForColumnValue('email', $email, $callback);
		$coleccionToken = $this->resendConfirmationForColumnValue('email', $email, $callback);
		$arreglo = ['token'=>$coleccionToken['token'], 'selector'=>$coleccionToken['selector'], 'alerta'=>null];
		return $arreglo;
	}
	...
	private function resendConfirmationForColumnValue($columnName, $columnValue, callable $callback) {
		...
		return $this->createConfirmationRequest($latestAttempt['user_id'],$latestAttempt['email'],$callback);
	}
