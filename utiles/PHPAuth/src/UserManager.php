...
abstract class UserManager {
	...
	protected function createUserInternal($requireUniqueUsername, $email, $password, $username = null, callable $callback = null) {
		...
		if ($verified === 0) {
			...
			// RETORNO PROPIO
			$coleccionTokens = $this->createConfirmationRequest($newUserId, $email, $callback);
		}

		// return $newUserId;
		//MODIFICAION PROPIA
		$arreglo = ['id'=>$newUserId, 'selector'=>$coleccionTokens['selector'], 'token'=>$coleccionTokens['token'], 'alerta'=>null];
		return $arreglo;
	}
	...
	protected function createConfirmationRequest($userId, $email, callable $callback) {
		...
		if (\is_callable($callback)) {
			...
			// MODIFICACION PROPIA
			return $arregloTokens = ['selector'=>$selector, 'token'=>$token];
		}
		else {
			throw new MissingCallbackError();
			// MODIFICACION PROPIA
			return $arregloTokens = ['selector'=>null, 'token'=>null];
		}
	}
